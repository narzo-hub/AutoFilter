...
# ============================
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '7123837674').split()] # Replace with the actual admin ID(s) to add
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1003030799577').split()]  # Channel id for auto indexing (make sure bot is admin)
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1002881534182'))  # Log channel id (make sure bot is admin)
BIN_CHANNEL = int(environ.get('BIN_CHANNEL', '-1002831115957'))  # Bin channel id (make sure bot is admin)
MOVIE_UPDATE_CHANNEL = int(environ.get('MOVIE_UPDATE_CHANNEL', '-1002805276505'))  # Notification of those who verify will be sent to your channel
...
DELETE_CHANNELS = [int(dch) if id_pattern.search(dch) else dch for dch in environ.get('DELETE_CHANNELS', '-1002720613611').split()]
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1002739655821')  # Support group id (make sure bot is admin)
...
#Auto approve 
CHAT_ID = [int(app_chat_id) if id_pattern.search(app_chat_id) else app_chat_id for app_chat_id in environ.get('CHAT_ID', '-1002995638706').split()]
...
# ============================
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://Yashds:Yashds@cluster0.huoi13r.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
DATABASE_URI2 = environ.get('DATABASE_URI2', "mongodb+srv://Yashds:yashds@yashds.xzuhxvo.mongodb.net/?retryWrites=true&w=majority&appName=Yashds")
...
# ============================
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/+0L8u-DadD_BiNDFl')
CHNL_LNK = environ.get('CHNL_LNK', 'https://t.me/+TMjaCOXnTZ0wZTc1')
OWNER_LNK = environ.get('OWNER_LNK', 'https://t.me/Bruce_Jaat_bot')
MOVIE_UPDATE_CHANNEL_LNK = environ.get('MOVIE_UPDATE_CHANNEL_LNK', 'https://t.me/+kQbHCh8YIDg5ZTM9')
OWNERID = int(os.environ.get('OWNERID', '7123837674'))  # Replace with the actual admin ID

# ============================
# User Configuration
# ============================
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '7123837674').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '7123837674').split()]
...
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', '-1002739655821')  # Support group link (make sure bot is admin)
...
FILE_STORE_CHANNEL = [int(ch) for ch in (environ.get('FILE_STORE_CHANNEL', '-1003030799577')).split()]
